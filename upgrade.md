...
## Завершено: Скрыть брендированную загрузку в режиме приватности (2024-12-16)
**Дата:** 2024-12-16

### Описание
При включённом режиме приватности не должна отображаться брендированная загрузка "Camroid M" - вместо неё показывается нейтральный спиннер.

### Выполненные изменения
- `App.tsx`: Изменён fallback в Suspense с `variant="branded"` на `variant="fullscreen"` для GamePage
- `game.tsx`: Заменён текст "Loading..." на нейтральный спиннер

### Технические детали
- `PageLoader variant="fullscreen"` показывает только простой спиннер без текста "Camroid M"
- `PageLoader variant="branded"` показывает анимацию с текстом "Camroid M" и прицелом
- При `settings.enabled && isLocked` теперь используется нейтральный загрузчик
