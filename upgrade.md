...
## Завершено: Полное скрытие брендинга в режиме приватности (2024-12-17)
**Дата:** 2024-12-17

### Описание
При включённом режиме приватности никакой брендинг "Camroid M" не должен быть виден до разблокировки. Заставка "Camroid M" появляется ТОЛЬКО после прохождения маскировочного приложения.

### Выполненные изменения
- `index.html`: 
  - Заменён `<title>Camroid M</title>` на `<title>2048</title>`
  - Заменены meta-теги на нейтральные (description, apple-mobile-web-app-title, application-name)
  - Заменены иконки с `/favicon.svg` на `/game-icon.svg`
- `App.tsx`: 
  - Вынесена логика в компонент `AppContent` внутри PrivacyProvider
  - Добавлена проверка `isConfigLoading` - пока конфиг загружается, показывается нейтральный спиннер
  - Решение о показе SplashScreen теперь принимается ПОСЛЕ загрузки конфига
  - Удалён отдельный Router компонент - логика объединена в AppContent
- `game.tsx`: Заменён текст "Loading..." на нейтральный спиннер

### Технические детали
- По умолчанию index.html показывает "2048" с иконкой игры
- `PageLoader variant="fullscreen"` показывает только простой спиннер без брендинга
- `PageLoader variant="branded"` показывает анимацию "Camroid M" - используется ТОЛЬКО после разблокировки
- Inline-скрипт в index.html обновляет метаданные на основе localStorage
- PrivacyProvider восстанавливает брендинг "Camroid M" только после успешной разблокировки

---

## Завершено: Обновление иконок игры 2048 (2024-12-16)
**Дата:** 2024-12-16
