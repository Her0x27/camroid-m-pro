...
**Mechanism:** Service Worker intercepts `/manifest.json` requests
**Data Source:** localStorage key `app-mode` (obfuscated with XOR + Base64)
**Update Flow:**
1. User selects privacy module in settings
2. Frontend saves encrypted config to localStorage
3. HTML storage event listener notifies Service Worker
4. Service Worker clears cache and regenerates manifest
5. Browser refreshes PWA metadata
...
- Version: 1.1.0
- Full configuration management via /api/config
...
**Cause:** Chrome caches manifest aggressively
**Workaround:** 
...
**Better Solution:** 
- Service Worker is updated automatically on each page reload
- Manifest cache headers set to no-cache, no-store, must-revalidate
- Consider implementing manifest versioning query parameter (?v=123)
